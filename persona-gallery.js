(() => {
  ...
  const kind = root.dataset.kind || 'pair';
  ...
  const categories = () => ['ALL', ...new Set(items.flatMap(i => i.categories || (i.pair ? [i.pair] : [])).filter(Boolean))];
  ...
  function draw(){
    const list = active==='ALL' ? items : items.filter(i => (i.categories||[i.pair]).includes(active));
    ...
    root.innerHTML = list.map((i,n)=>`<article class="pg-card ${kind==='comic'?'pg-card--comic':''}">
      ...
      <div class="pg-card__body"><div class="pg-card__meta">${esc(i.pair || i.characters || `EPISODE ${String(n+1).padStart(3,'0')}`)}</div><h2>${esc(i.title || 'UNTITLED')}</h2><p>${esc(i.description || '')}</p></div></article>`).join('');
